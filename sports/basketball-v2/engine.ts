...
import { getGameById, getRecentFinishedGames, getHeadToHead, type CanonicalGame } from './warehouse/games-repo';
...
export async function predictGameV2(
  ...
  // 1. Fetch game from warehouse
  const game = await getGameById(gameId);
  if (!game) {
    throw new Error(`v2 warehouse miss: game ${gameId} not found`);
  }
