...
import { parseBasketballOdds, type BasketballRawOdds } from './advanced-odds-parser';
import { buildEnhancedMarkets, type BasketballEnhancedMarkets } from './enhanced-prediction';
...
interface BasketballPrediction {
  ...
  // NEW: Full 24-market parsed odds (advanced-odds-parser output)
  raw_odds: BasketballRawOdds | null;
  // NEW: Enhanced predictions using period-level, HTFT, handicap, team totals
  enhanced_markets: BasketballEnhancedMarkets | null;
...
export class BasketballPredictionEngine {
  ...
  static async generatePrediction(
    ...
    // 2b. Parse real odds from API-Basketball (legacy 5-market parser)
    const parsedOdds = parseOddsResponse(rawOddsData);
    // 2c. Advanced parser — full 24 bet types (period O/U, HTFT, AH, team totals, odd/even)
    const rawOddsAdvanced = parseBasketballOdds(rawOddsData);
    ...
    // ─────────────────────────────────────────────────────────────────────
    // ADVANCED MARKETS: Period analyses (1H, 2H, Q1), HTFT, handicaps,
    // team totals, odd/even, value bets. Uses the full 24 bet types.
    // ─────────────────────────────────────────────────────────────────────
    const enhancedMarkets = buildEnhancedMarkets({
      expectedHomePoints: adjustedHomePoints,
      expectedAwayPoints: adjustedAwayPoints,
      homePointsStdDev: teamStdDev,
      awayPointsStdDev: teamStdDev,
      rawOdds: rawOddsAdvanced,
    });
    ...
    const prediction: BasketballPrediction = {
      ...
      raw_odds: rawOddsAdvanced,
      enhanced_markets: enhancedMarkets,
