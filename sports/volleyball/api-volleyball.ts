...
/**
 * Major volleyball leagues with their API-Volleyball IDs
 */
export const MAJOR_VOLLEYBALL_LEAGUES = {
  CEV_CHAMPIONS_LEAGUE: 78,
  SERIE_A_ITALY: 85,
  BUNDESLIGA_GERMANY: 90,
  SUPER_LEAGUE_TURKEY: 32,
  LIGUE_A_FRANCE: 72,
  PLUSLIGA_POLAND: 44,
  SUPER_LEAGUE_RUSSIA: 94,
  SERIE_A1_BRAZIL: 26,
} as const;

/**
 * Volleyball API service using API-Volleyball (api-sports.io)
 * Base URL: https://v1.volleyball.api-sports.io
 *
 * Wraps BaseSportApiClient with volleyball-specific methods
 * for fetching games, standings, head-to-head, and team statistics.
 */
export class ApiVolleyballService extends BaseSportApiClient {
  constructor() {
    super('volleyball');
  }

  /**
   * Fetch all volleyball games for a given date.
   ...
   * @param id - Game ID from API-Volleyball
   */
  ...
  /**
   * Fetch live volleyball games currently in progress.
   */
  ...
  /**
   * Determine the current season string for volleyball leagues.
   * Most volleyball seasons start in September/October and run through spring.
   */
  getCurrentSeason(): string {
    const now = new Date();
    const year = now.getFullYear();
    const month = now.getMonth() + 1;
    // Most volleyball seasons start in September/October
    if (month >= 9) {
      return `${year}-${year + 1}`;
    }
    return `${year - 1}-${year}`;
 ...
 */
export const volleyballApi = new ApiVolleyballService();
