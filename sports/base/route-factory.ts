...
import { trackSportPrediction } from '@/lib/probet/multi-sport-tracker';
...
export function createPredictionHandler(
  ...
  return async function handler(
    ...
    try {
      ...
      // ═════════════════════════════════════════════════════════════════
      // Auto-track prediction in the unified tracking DB.
      // Fire-and-forget — never blocks the response.
      // Works for basketball, hockey, volleyball, handball.
      // ═════════════════════════════════════════════════════════════════
      if (sport !== 'football' && result) {
        trackSportPrediction(sport as any, result);
      }

      // Attach resolution status (win/loss markers) if available
      let resolution: any = undefined;
      try {
        const status = await getPredictionStatus(sport, gameId);
        if (status.status !== 'unknown') {
          resolution = status;
        }
      } catch {
        // Tracking DB may be unavailable; ignore
      }
