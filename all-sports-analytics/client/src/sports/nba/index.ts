...
import { analyzeNbaDeep, type NbaDeepAnalysis } from './nbaPlayers';
...
function predict(params: {
  ...
  // ===== NBA DEEP ANALYSIS: pace + rest days =====
  const homeDeep = analyzeNbaDeep({
    teamStats: homeStats,
    avgScore: baseHome,
    game,
    recentGames: h2h,
  ...
  // Expected scores with pace + rest adjustments
  let meanHome = Math.max(60, ((homeAvg.scored + awayAvg.conceded) / 2) * adv * Math.pow(formFactor, 0.15) + h2hAdjust);
  let meanAway = Math.max(60, ((awayAvg.scored + homeAvg.conceded) / 2) / Math.pow(formFactor, 0.08) - h2hAdjust * 0.5);

  // Apply deep analysis adjustments
  meanHome += homeDeep.totalScoreAdjustment;
  meanAway += awayDeep.totalScoreAdjustment;
  meanHome = Math.max(60, meanHome);
  meanAway = Math.max(60, meanAway);
