    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative z-10 h-full flex flex-col justify-center px-6 sm:px-8">
          <h1 className="font-display text-2xl sm:text-3xl font-bold tracking-wider neon-text">
            {t('home_title')}
          </h1>
          ...
          <div className="flex gap-4 mt-3">
            ...
            <StatBadge icon={<Clock className="w-3.5 h-3.5" />} label={t('home_upcoming')} value={upcomingGames.length} color="text-ice" />
            <StatBadge icon={<TrendingUp className="w-3.5 h-3.5" />} label={t('home_finished')} value={finishedGames.length} color="text-aurora" />
          </div>
        </div>
      </div>
...
function GameCard({ game, delay }: { game: Game; delay: number }) {
  ...
  return (
    <Link
      href={`/match/${game.id}`}
      className={`block glass-card glass-card-hover rounded-lg p-3 sm:p-4 transition-all duration-300 animate-fade-in-up ${live ? "border-l-2 border-l-destructive" : ""}`}
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        <div className="w-16 sm:w-20 text-center shrink-0">
          {live ? (
            ...
            <span className="text-sm font-mono text-ice">{formatTime(game.date)}</span>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`text-lg font-bold font-mono w-8 text-center ${live ? "neon-text" : finished ? "text-foreground" : "text-muted-foreground"}`}>
