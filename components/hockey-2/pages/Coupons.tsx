      ...
      <div className="space-y-6">
        <div className="relative rounded-xl overflow-hidden p-8 sm:p-12" style={{ backgroundImage: `url(${HERO_BG})`, backgroundSize: "cover", backgroundPosition: "center" }}>
          ...
          <div className="relative z-10">
            <h1 className="font-bold text-3xl sm:text-4xl font-black text-cyan-600 dark:text-cyan-400 font-bold mb-2">{t('coupons_title')}</h1>
            <p className="text-sm text-muted-foreground">{t('coupons_subtitle')}</p>
          </div>
        </div>

        <div className="bg-card border border-border rounded-xl backdrop-blur-sm rounded-xl p-8">
    ...
    <div className="space-y-6">
      ...
      <div className="relative rounded-xl overflow-hidden p-8 sm:p-12" style={{ backgroundImage: `url(${HERO_BG})`, backgroundSize: "cover", backgroundPosition: "center" }}>
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Link href="/hockey-2/coupon-history" className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/40 hover:bg-emerald-500/30 transition-colors">
      ...
      {strategies.length === 0 ? (
        ...
        <div className="space-y-4">
          <h2 className="font-bold text-lg font-bold tracking-wider text-cyan-600 dark:text-cyan-400 font-bold flex items-center gap-2">
          ...
          {strategies.map((strategy, idx) => {
            ...
            return (
              <div key={idx} className="bg-card border border-border rounded-xl backdrop-blur-sm rounded-xl p-5 border border-cyan-500/20">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      <h3 className="font-bold text-base font-bold text-cyan-600 dark:text-cyan-400 font-bold">{strategy.name}</h3>
                      <RiskBadge risk={strategy.riskLevel === 'very-high' ? 'high' : strategy.riskLevel} />
                      {isSaved && (
                        <span className="text-[10px] px-2 py-0.5 rounded-full border border-emerald-500/40 text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 font-bold flex items-center gap-1">
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold font-black text-emerald-600 dark:text-emerald-400 font-bold">{strategy.totalOdds.toFixed(2)}</p>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4 pb-4 border-b border-border/30">
                  <div>
                    ...
                    <p className="text-sm font-mono font-bold text-cyan-600 dark:text-cyan-400">{(strategy.expectedProbability * 100).toFixed(1)}%</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">{t('coupons_expected_value')}</p>
                    <p className={`text-sm font-mono font-bold ${strategy.expectedValue > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'}`}>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-mono font-bold text-emerald-600 dark:text-emerald-400 font-bold">{(strategy.suggestedStake * strategy.totalOdds).toFixed(0)} {t('coupons_unit')}</p>
                  </div>
                </div>
                  ...
                  {strategyBets.map((bet, i) => {
                    ...
                    return (
                      <div key={i} className={`flex items-center justify-between text-xs p-2 rounded-lg ${
                        betResult === 'won' ? 'bg-emerald-500/10 border border-emerald-500/30' :
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="font-mono font-bold text-cyan-600 dark:text-cyan-400">{bet.odds.toFixed(2)}</span>
                ...
                <button onClick={() => {
                  ...
                  className={`w-full py-2 rounded-lg text-sm font-semibold transition-all flex items-center justify-center gap-2 ${
                    isSaved ? "bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/40" : "bg-cyan-500 text-background hover:bg-cyan-500/90"
                  }`}>
      ...
      {analyses.length > 0 && (
        <div className="space-y-4">
          ...
          {analyses.map((analysis) => {
            ...
            return (
              <div key={analysis.game.id} className="bg-card border border-border rounded-xl backdrop-blur-sm rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Link href={`/hockey-2/match/${analysis.game.id}`} className="text-cyan-600 dark:text-cyan-400 hover:text-emerald-600 dark:text-emerald-400 transition-colors">
                  ...
                  {analysis.valueBets.slice(0, 5).map((vb, i) => {
                    ...
                    return (
                      <div key={i} className={`flex items-center justify-between text-xs p-2 rounded-lg ${
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="font-mono font-bold text-cyan-600 dark:text-cyan-400">{vb.odds.toFixed(2)}</span>
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold">+{vb.edge.toFixed(1)}%</span>
                          {betResult !== 'pending' && <ResultBadge result={betResult} />}
                        </div>
                      </div>
                    );
      ...
      {/* Disclaimer */}
      <div className="bg-card border border-border rounded-xl backdrop-blur-sm rounded-xl p-4 border border-warning/30 bg-warning/5">
