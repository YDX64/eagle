...
import { calculateEloFromStandings, type EloRating } from "@/lib/hockey-2/analysis";
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative z-10 h-full flex flex-col justify-center px-6">
          <h1 className="font-bold text-2xl font-bold tracking-wider text-cyan-600 dark:text-cyan-400 font-bold">{t('standings_title')}</h1>
        ...
        {POPULAR_LEAGUES.map((l) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
              selectedLeague === l.id ? "bg-card border border-border rounded-xl backdrop-blur-sm border-cyan-500/40 shadow-sm text-cyan-600 dark:text-cyan-400" : "text-muted-foreground hover:text-foreground hover:bg-accent/50"
            }`}
          >
            {l.name}
          </button>
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${!showElo ? "bg-card border border-border rounded-xl backdrop-blur-sm border-cyan-500/40 shadow-sm text-cyan-600 dark:text-cyan-400" : "text-muted-foreground hover:bg-accent/50"}`}
        >
  ...
  return (
    <div className="space-y-6">
      {groups.map((group, gi) => {
        ...
        return (
          <div key={gi} className="bg-card border border-border rounded-xl backdrop-blur-sm rounded-xl overflow-hidden">
                      ...
                      <tr key={s.team.id} className="border-b border-border/20 hover:bg-accent/20 transition-colors">
                        ...
                        <td className="py-2 px-2">
                          <div className="flex items-center gap-2">
                            ...
                            {s.description === "Playoffs" && <span className="text-[8px] text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-1 rounded">PO</span>}
                          </div>
                        </td>
                        <td className="text-center py-2 px-2">{s.games.played}</td>
                        <td className="text-center py-2 px-2 text-emerald-600 dark:text-emerald-400">{s.games.win.total}</td>
                        ...
                        <td className={`text-center py-2 px-2 ${gd > 0 ? "text-emerald-600 dark:text-emerald-400" : gd < 0 ? "text-destructive" : ""}`}>
                          {gd > 0 ? `+${gd}` : gd}
                        </td>
                        <td className="text-center py-2 px-2 font-bold text-cyan-600 dark:text-cyan-400">{s.points}</td>
...
function EloTable({ ratings }: { ratings: EloRating[] }) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl backdrop-blur-sm rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-border bg-accent/20">
            ...
            {ratings.map((r, i) => {
              ...
              return (
                <tr key={r.teamId} className="border-b border-border/20 hover:bg-accent/20 transition-colors">
                  ...
                  <td className="text-center py-2 px-2 font-bold text-cyan-600 dark:text-cyan-400">{r.rating}</td>
                  <td className="text-center py-2 px-2">
                    {r.trend === "up" && <TrendingUp className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 inline" />}
      ...
      {form.split("").slice(-5).map((f, i) => (
        <span
          ...
          className={`w-4 h-4 rounded-sm flex items-center justify-center text-[8px] font-bold ${
            f === "W" ? "bg-emerald-500/20 text-emerald-600 dark:text-emerald-400" : f === "L" ? "bg-destructive/20 text-destructive" : "bg-warning/20 text-warning"
          }`}
        >
          {f}
        </span>
