...
const SPORTS = [
  ...
  { key: 'hockey-2', label: 'Hockey 2', icon: '🏒', href: '/hockey-2', color: 'cyan' },
  { key: 'all-sports', label: 'Tüm Sporlar', icon: '🏆', href: '/all-sports', color: 'emerald' },
] as const;

const TOOLS = [
  { key: 'bulk', label: 'Toplu Analiz', href: '/bulk-analysis' },
  { key: 'stats', label: 'Istatistikler', href: '/statistics' },
  { key: 'history', label: 'Tahmin Gecmisi', href: '/prediction-history' },
  { key: 'hot', label: 'Yuksek Oran', href: '/high-odds-winners' },
  { key: 'iddaa-hockey', label: 'iddaa Buz Hokeyi', href: '/iddaa-hockey' },
] as const;
...
const colorMap: Record<string, { active: string; hover: string; text: string }> = {
  ...
  cyan: { active: 'bg-cyan-600 text-white shadow-lg shadow-cyan-500/30', hover: 'hover:bg-cyan-100 dark:hover:bg-cyan-900/30', text: 'text-cyan-700 dark:text-cyan-400' },
};
...
export function SportSelector() {
  ...
  const getActiveSport = () => {
    ...
    if (pathname.startsWith('/hockey-2')) return 'hockey-2';
    if (pathname.startsWith('/all-sports')) return 'all-sports';
