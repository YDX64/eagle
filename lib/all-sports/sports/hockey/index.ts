/**
 * Hockey (Ice Hockey) Sport Plugin
 * v1.hockey.api-sports.io
 *
 * Karakteristik:
 * - Orta-yüksek skorlu (~6 gol toplam)
 * - Üç periyot (60 dk)
 * - Regülasyon beraberlik mümkün
 * - NHL/KHL'de OT/SO karar verir
 *
 * API'nin yapısı futbolun aksine flat: game.id, game.status.short, game.teams.home.id gibi
 */
...
function predict(params: {
  ...
  // Hockey team statistics - fields mirror api-sports format
  if (homeStats?.goals) {
  ...
  // Hockey-specific lines: higher OU range, wider handicap spread
