...
async function nbaFetch<T>(
  ...
  // api-sports returns HTTP 200 even on rate limit / subscription errors;
  // the error lives inside json.errors as an object. Detect it and throw so
  // callers (daily-cron, etc.) can back off instead of silently receiving [].
  if (json.errors && !Array.isArray(json.errors) && Object.keys(json.errors).length > 0) {
    const errObj = json.errors as Record<string, string>;
    const firstKey = Object.keys(errObj)[0];
    const errMsg = errObj[firstKey] || JSON.stringify(errObj);
    if (/rate.?limit/i.test(firstKey) || /too many|exceeded the limit/i.test(errMsg)) {
      _logRateLimitOnce(errMsg);
      throw new Error(`NBA API rate limited: ${errMsg}`);
    }
    console.error('[api-nba] errors:', json.errors);
    throw new Error(`NBA API ${endpoint}: ${firstKey}=${errMsg}`);
  }
...
// Rate-limit log throttle: only emit one console.warn per minute to avoid
// flooding container logs when the daily cron loops over many games.
let _lastRateLimitLogMs = 0;
function _logRateLimitOnce(msg: string): void {
  const now = Date.now();
  if (now - _lastRateLimitLogMs > 60_000) {
    _lastRateLimitLogMs = now;
    console.warn(`[api-nba] rate limit hit: ${msg} (suppressing further warnings for 60s)`);
  }
}
