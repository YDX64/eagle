...
// Tüm istekler probet sunucusundaki proxy'den geçer; api-sports anahtarı
// burada değil, sunucu ortamında (/api/hockey-2/proxy/*) kalır.
const API_BASE =
  typeof window === 'undefined'
    ? 'http://127.0.0.1:5000/api/hockey-2/proxy'
    : '/api/hockey-2/proxy';
...
async function fetchApi<T>(endpoint: string, params?: Record<string, string | number>): Promise<ApiResponse<T>> {
  // API_BASE bir relative path ("/api/hockey-2/proxy") olduğu için
  // new URL() constructor'u çalışmaz (absolute URL ister). URL'i string
  // olarak kendimiz kuruyoruz; URLSearchParams ile query string encode.
