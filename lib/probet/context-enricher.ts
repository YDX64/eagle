...
export interface LiveRawOdds {
  ...
  ah_home_plus_1: number | null;
  ah_away_plus_1: number | null;
  ah_away_minus_1: number | null;
...
const EMPTY_RAW_ODDS: LiveRawOdds = {
  ...
  ah_home_minus_1: null, ah_home_plus_1: null, ah_away_plus_1: null, ah_away_minus_1: null,
  correct_scores: {},
};
...
function processOdds(oddsResponse: any[]): Partial<ContextExtras> {
  ...
  const collected: Record<string, number[]> = {
    ...
    ah_home_minus_1: [], ah_home_plus_1: [], ah_away_plus_1: [], ah_away_minus_1: [],
  };
    ...
    for (const bet of bm.bets || []) {
      ...
      if (name === 'asian handicap') {
        for (const v of values) {
          ...
          else if (valRaw === 'away -1' || valRaw === 'away (-1.0)') collected.ah_away_minus_1.push(odd);
          else if (valRaw === 'home +1' || valRaw === 'home (+1.0)') collected.ah_home_plus_1.push(odd);
        }
      }
    }
  }
  ...
  const rawOdds: LiveRawOdds = {
    ...
    ah_home_plus_1: averageOdds(collected.ah_home_plus_1),
    ah_away_plus_1: averageOdds(collected.ah_away_plus_1),
    ah_away_minus_1: averageOdds(collected.ah_away_minus_1),
    correct_scores: {},
  };
  // Average HTFT
